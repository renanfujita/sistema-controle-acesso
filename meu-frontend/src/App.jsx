...
function AdminPage({ currentUser, users, setUsers, onLogout }) {
  ...
  const criarUsuario = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        setMsg("Erro: Este nome de usuário já existe no banco.");
      ...
      setShowNew(false);
      setMsg("Usuário salvo no banco com senha criptografada!");
      setTimeout(() => setMsg(""), 4000);

    } catch (error) {
      setMsg("Erro de conexão com o servidor Back-end.");
      setTimeout(() => setMsg(""), 3000);
    }
  };
